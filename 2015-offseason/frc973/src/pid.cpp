#include "WPILib.h"
#include "pid.hpp"
#include <pthread.h>
#include <unistd.h>

namespace frc973 {

PID::PID(float p_, float i_, float d_) {
    createObject(p_, i_, d_);
}

PID::PID(float gains[3]) {
    createObject(gains[0], gains[1], gains[2]);
}

void PID::createObject(float p_, float i_, float d_) {
    p = p_;
    i = i_;
    d = d_;

    target = 0;
    min = -1;
    max = 1;

    integral = 0;
    iterm = 0;
    icap = 0;
    derivative = 0;

    pterm = 0;
    dterm = 0;

    output = 0;
    prevErr = 0;

    err = 0;

    timer = new Timer();

    reset();
}

void PID::createThread() {
    pthread_t thread;
}

void PID::setGains(float p_, float i_, float d_) {
    updateGains(p_, i_, d_);
}

void PID::setGains(float gains[3]) {
    updateGains(gains[0], gains[1], gains[2]);
}

void PID::updateGains(float p_, float i_,float d_) {
    p = p_;
    i = i_;
    d = d_;
}

void PID::setTarget(float target_) {
    target = target_;
}

float PID::getTarget() {
    return target;
}

void PID::reset() {
    target = 0;
    integral = 0;
    output = 0;
    prevErr = 0;
    timer->Reset();
}

void PID::start() {
    timer->Start();
    timer->Reset();
}

void PID::stop() {
    timer->Stop();
}

void PID::setICap(float icap_) {
    icap = icap_;
}

void PID::setBounds(float min_, float max_) {
    min = min_;
    max = max_;
}

float PID::update(float actual) {
    return calculateOutput(actual, timer);
}

float PID::update(float actual, Timer *t) {
    return calculateOutput(actual, t);
}

float PID::calculateOutput(float actual, Timer *t) {
    err = target - actual;

    // Calculate integral
    integral = integral + err * t->Get();
    iterm = i * integral;
    if (icap != 0) {
        if (iterm > icap) {
            integral = icap/i;
        }
        else if (iterm < -icap) {
            integral = -icap/i;
        }
        iterm = i * integral;
    }

    // Calculate derivative
    derivative = (err - prevErr) / t->Get();

    pterm = p * err;

    dterm = d * derivative;

    output = pterm + iterm + dterm;
    prevErr = err;

    if (output > max) {
        output = max;
    }
    if (output < min) {
        output = min;
    }

    return output;
}

float PID::getP() {
    return pterm;
}

float PID::getI() {
    return iterm;
}

float PID::getD() {
    return dterm;
}

}
